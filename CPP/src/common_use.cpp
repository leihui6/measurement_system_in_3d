...
void export_measured_data(std::multimap<std::string, std::string>& measurement_pairs_map, std::vector<measurement_value>& mv_vec, const std::string & output_file_name)
{
	...
	for (it = measurement_pairs_map.begin(); it != measurement_pairs_map.end(); ++it)
	{
		...
		if (mv_vec[i].distance_geometrical == INVALIDVALUE)
			ofile << INVALIDVALUE <<" ";
		else
			ofile << mv_vec[i].distance_geometrical << " ";
		
		if (mv_vec[i].distance_scattered == INVALIDVALUE)
			ofile << INVALIDVALUE << " ";
		else
			ofile << mv_vec[i].distance_scattered << " ";

		if (mv_vec[i].angle == INVALIDVALUE)
			ofile << INVALIDVALUE << " ";
		else
			ofile << mv_vec[i].angle << " ";
