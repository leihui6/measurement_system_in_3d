...
void kd_tree::search_points_correspondence(std::vector<point_3d>& points, std::vector<point_3d>& found_points)
{
	for (size_t i = 0; i < points.size(); ++i)
	{
		std::vector<size_t> ret_index;
		...
		if (ret_index.size() == 1)
		{
			found_points.push_back(get_point(ret_index.front()));
		}
		else if (ret_index.empty())
		{
			// error
...
void kd_tree::search_points_correspondence(std::vector<point_3d>& points)
{
	std::vector<point_3d> new_points;
	...
	for (size_t i = 0; i < points.size(); ++i)
	{
		...
		if (ret_index.size() == 1)
		{
			new_points.push_back(get_point(ret_index.front()));
		}
		else if (ret_index.empty())
		{
			// error
